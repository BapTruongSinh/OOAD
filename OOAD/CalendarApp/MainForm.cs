...
namespace CalendarApp
{
    public partial class MainForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            _appointmentList.Columns.Add("View", 140); // For viewing participants
        ...
        private void AppointmentList_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            if (hitTest.Item != null && hitTest.SubItem != null)
            {
                ...
                else if (subItemIndex == 7 && appointment is GroupMeeting)
                {
                    ...
                    {
                        // Tạo form mới để hiển thị danh sách người tham gia
                        Form participantsForm = new Form
                        {
                            Text = "Thông tin người tham gia cuộc họp",
                        ...
                        {
                            Text = $"Danh sách người tham gia cuộc họp '{groupMeeting.Name}':",
                            Font = new Font("Arial", 10, FontStyle.Bold),
                            Location = new Point(70, 20),
                            Size = new Size(380, 40)
                        };
                        
                        // Tạo ListView để hiển thị danh sách người tham gia
                        ...
                        participantsListView.Columns.Add("ID", 50);
                        participantsListView.Columns.Add("Tên người dùng", 200);
                        participantsListView.Columns.Add("Vai trò", 140);
                        
                        // Lấy người tạo cuộc họp
                        var creator = groupMeeting.Owner;
                        if (creator != null)
                        {
                            var creatorItem = new ListViewItem(creator.Id.ToString());
                            creatorItem.SubItems.Add(creator.Name);
                            creatorItem.SubItems.Add("Người tạo");
                            creatorItem.BackColor = Color.LightYellow;
                            participantsListView.Items.Add(creatorItem);
                        }
                        ...
                        var participants = dbContext.GroupMeetingParticipants
                            .Where(p => p.GroupMeetingId == groupMeeting.Id && p.ParticipantId != groupMeeting.OwnerId)
                            .Select(p => p.Participant)
                            .ToList();
                        
                        // Thêm người tham gia vào ListView
                        ...
                        participantsForm.Controls.Add(infoIcon);
                        participantsForm.Controls.Add(titleLabel);
                        participantsForm.Controls.Add(participantsListView);
                        participantsForm.Controls.Add(btnOK);
                        participantsForm.AcceptButton = btnOK;
